...
import {
  hpgTokenCOntractAbi,
  hpgTokenContractAddress,
} from "../Utils/hpgTokens";
  ...
  const getUserBalance = async () => {
    ...
    try {
      if (
        ...
        let hpgContractOf = new web3.eth.Contract(
          hpgTokenCOntractAbi,
          hpgTokenContractAddress
        );
  ...
  const buyTokens = async () => {
    ...
    try {
      if (acc == "No Wallet") {
        ...
        let flag = false;
        let hpgTokenContractOf = new web3.eth.Contract(
          hpgTokenCOntractAbi,
          hpgTokenContractAddress
        );
        let busdTokenContractOf = new web3.eth.Contract(
          busdTokenAbi,
          busdTokenAddress
        );
        let contractBal = await hpgTokenContractOf.methods
          .balanceOf(smsIdoContractAddress)
          .call();
        ...
        if (isAdd != fakeAddress) {
          ...
          const isAddagain = await contractOf.methods
            .CheckReferrals(userEntered)
            .call();
            ...
            if (contractBal > estimatedToken) {
              if (flag == true) {
                if (userEntered == acc) {
                  ...
                  await busdTokenContractOf.methods
                    .approve(smsIdoContractAddress, passingVar)
                    .send({
                      from: acc,
                    });
                  toast.success("Approved Successfully");
                  await contractOf.methods
                    .buyTokens(passingVar, userEntered)
                    .send({
                      from: acc,
                    });
    ...
    try {
      if (
        ...
        if (refAddress != fakeAddress) {
          ...
          console.log(
            "refAddress != fakeAddress",
            `${window.location.host}/presale/?ref=${acc}`
          );
          setShowRef(referral);
        } else {
          ...
          let ownAdd = await contractOf.methods.owner().call();
          if (code == null) {
            setShowRef(`${window.location.host}/presale/?ref=${ownAdd}`);
          } else {
            let refAdd = await contractOf.methods.getaddress(code).call();
            if (fakeAddress != refAdd) {
              setShowRef(`${window.location.host}/presale/?ref=${code}`);
            } else {
              setShowRef(`${window.location.host}/presale/?ref=${ownAdd}`);
            }
          }
  ...
  const setReferralAddress = async () => {
    try {
      ...
      if (code == null) {
        ...
        const fakeAddress = "0x0000000000000000000000000000000000000000";
        let refAdd = await contractOf.methods.getaddress(code).call();
        console.log("ref ", refAdd);
        if (fakeAddress != refAdd) {
          setOwnerAddress(`${window.location.host}/presale/?ref=${code}`);
        } else {
          setOwnerAddress(`${window.location.host}/presale/?ref=${ownAdd}`);
        }
      }
    } catch (error) {}
  };
          ...
          <div className="row mt-5 d-flex justify-content-evenly g-0">
              ...
              <div className=" mintCardBody d-flex justify-content-start">
                <ul className="">
                  ...
                  <li>
                    ...
                    <a
                      className="textColor"
                      onClick={() => userOwnerAsReferral()}
                    >
                      {" "}
                      {ownerAddress?.substring(0, 4) +
                        "..." +
                        ownerAddress?.substring(ownerAddress?.length - 4)}
                    </a>{" "}
                    as a referral if you have got no Referral.
                  </li>
                </ul>
              </div>
              {/* <div className="p-3">
            ...
            <div className="col-lg-4 col-md-12 col-sm-11  d-flex flex-column   mintCard2 p-2 mt-3">
              ...
              <div className="mintTotal">
                <div>
                  <span className="totalSpan ps-2">Min Buying</span>
                </div>
                <div>
                  <span className="KLAYspan pe-2">
              ...
              <div className=" row">
                <span className="KLAYspan pe-2">My Referral Address</span>
              </div>
              <div className="preslaesAdditionSection">
                <input
                  disabled
              ...
              <div className="preslaesAdditionSection mt-4 ">
                ...
                <button className="btnPlus btnPlusPresale" onClick={handlePlus}>
                  +
                </button>
              </div>
